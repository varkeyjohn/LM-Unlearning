...
def unlearn(model, clean_loader, poisoned_loader, alpha, num_epochs, learning_rate, device):
    ...
    for epoch in range(num_epochs):
        ...
        num_batches = 0
        poisoned_iter = cycle(poisoned_loader)  # Cycle through poisoned loader to match clean loader
        for clean_batch in tqdm(clean_loader, desc=f"Epoch [{epoch+1}/{num_epochs}]"):
            ...
            poisoned_input_ids, poisoned_attention_mask, poisoned_labels = poisoned_batch
            ...
            poisoned_attention_mask = poisoned_attention_mask.to(device, non_blocking=True)
...
if __name__ == "__main__":
    ...
    # Load model
    model.load_state_dict(torch.load("saved_models/posioned_model_final.pth"))
    ...
    backdoor_test_loader = DataLoader(backdoor_test_dataset, batch_size=batch_size, shuffle=False, collate_fn=collate_fn)
